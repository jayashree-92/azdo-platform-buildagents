...
#---------------------------------------------------------
# ACI Windows from module
#---------------------------------------------------------
module "aci-windows" {
  source = "git::ssh://git@ssh.dev.azure.com/v3/Innocap/Terraform-Modules/terraform-azurerm-container-group//module//?ref=v2.0.1"
  count  = try(local.default_windows_container_config.azdo_number_of_agents, 0)
  ...
  location_code       = var.location_code
  environment_code    = var.environment_code
  ip_address_type     = local.default_windows_container_config.ip_address_type
  os_type             = "Windows"
  subnet_ids          = [data.azurerm_subnet.snet.id]
  ...
  containers = [
    {
      name   = local.default_windows_container_config.containers.default.name
      image  = "${data.azurerm_container_registry.acr.login_server}/${local.default_windows_container_config.containers.default.name}:${local.default_windows_container_config.containers.default.image}"
      cpu    = local.default_windows_container_config.containers.default.cpu
      memory = local.default_windows_container_config.containers.default.memory

      ports = [
        {
          port     = local.default_windows_container_config.containers.default.ports.default.port
          protocol = local.default_windows_container_config.containers.default.ports.default.protocol
        }
      ]

      secure_environment_variables = {
        "AZP_URL"        = local.default_windows_container_config.azdo_url
        "AZP_TOKEN"      = local.default_windows_container_config.azdo_pat_token
        "AZP_AGENT_NAME" = format("windowsagent-%s-%s", local.full_env_code, count.index)
        "AZP_POOL"       = local.default_windows_container_config.azdo_agent_pool_name
      }
    }
  ]
  depends_on = [
